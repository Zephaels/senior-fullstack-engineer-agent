...
ROOT=Path(__file__).resolve().parents[1]; NAME='senior-fullstack-engineer-agent'; VERSION=(ROOT/'VERSION').read_text().strip(); SRC=ROOT/'source'/NAME; PLUGIN=ROOT/'plugins'/NAME
IGNORED_DIRS={'dist','.git','__pycache__','.pytest_cache','.mypy_cache','qualification-results'}
...
for skill_dir in [PLUGIN/'skills'/p.parent.name for p in plugin_skills]:
 ...
 try:
  d=yaml.safe_load(y.read_text()) or {}; interface=d.get('interface',{})
...
for p in list(ROOT.rglob('*.json'))+list(ROOT.rglob('*.yaml'))+list(ROOT.rglob('*.yml')):
 if ignored(p): continue
 try: d=json.loads(p.read_text()) if p.suffix=='.json' else yaml.safe_load(p.read_text())
...
manifest=json.loads((SRC/'evals/trigger/manifest.json').read_text())
...
for skill in expected:
 ...
 cases=json.loads(p.read_text()); pos=sum(bool(c['should_trigger']) for c in cases); neg=len(cases)-pos
 if pos!=neg: bad('trigger-balance',f'{skill}: {pos}/{neg}')
# Versions.
manifest_plugin=json.loads((PLUGIN/'.codex-plugin/plugin.json').read_text())
for label,val in [('plugin',manifest_plugin.get('version')),('source',(SRC/'VERSION').read_text().strip())]:
...
for base in [SRC,PLUGIN]:
 ...
 try:
  md=json.loads(mp.read_text()); actual={x['path']:x['sha256'] for x in md['files']}; expected_files=manifest_expected(base)
...
for p in [ROOT/'README.md',SRC/'README.md',PLUGIN/'README.md']:
 if p.exists() and re.search(r'\bv1\.0\.0 GA\b|General Availability build',p.read_text(),re.I): bad('misleading-ga-label',str(p))
