...
import hashlib, json
ROOT=Path(__file__).resolve().parents[1]; VERSION=(ROOT/'VERSION').read_text().strip(); NAME='senior-fullstack-engineer-agent'
EXCLUDED_DIRS={'dist','.git','__pycache__','.pytest_cache','.mypy_cache','qualification-results'}
def iter_files(base,skip_names):
 for p in sorted(base.rglob('*')):
  ...
  yield p
def build(base,manifest_path,kind):
 rows=[]
 ...
 data={'schema_version':'1.0','kind':kind,'version':VERSION,'root':base.name,'file_count':len(rows),'files':rows}
...
build(ROOT,ROOT/'MANIFEST.json','release-repository'); sbom(ROOT,ROOT/'SBOM.spdx.json',NAME+'-release')
